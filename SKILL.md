...
这个 skill 用于当前仓库的 dev 部署流程，但能力安装在全局目录，方便跨项目复用。
...
```bash
bash "$CODEX_HOME/skills/yunxiao-dev-deploy/scripts/dev_deploy.sh" run --pipeline-link "https://flow.aliyun.com/pipelines/123456/current"
```
...
```bash
bash "$CODEX_HOME/skills/yunxiao-dev-deploy/scripts/dev_deploy.sh" run --wait
```
