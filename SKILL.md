...
9. 把当前分支加入分支列表，去重后触发 dev 部署；如果 403，明确提示需要 `流水线运行实例 / 读写`；如果本次参数会删掉最近一次成功部署里的已有分支，默认直接失败
10. 如果运行详情里出现阻塞动作，先区分是不是 `CONFLICT_MERGE`。如果是分支冲突，默认只支持“在原分支解决”这条路径
11. 在原分支解决时，先取消这个阻塞 run，避免留下悬挂的 `RUNNING/WAITING` 实例；然后拉取 `releaseBranch` 到本地做合并演练，确认具体冲突文件并在当前分支解决，最后重新触发部署
12. 如果判断这次冲突必须直接改 `releaseBranch` 才能继续，则停止自动处理并明确报告给用户；不要把“直接修改 `release/xxx` 并继续 pipeline”当成 skill 默认能力
13. 如果是人工执行、人工确认等非冲突阻塞，再提醒用户去云效页面处理，不继续盲等
14. 默认不阻塞等待；如果用户明确要求跟进部署结果，再调用等待脚本，或优先使用 subagent 轮询
...
- 再把当前分支追加进去
- 去重后作为本次运行参数
- 如果新分支集比最近一次成功部署更小，默认直接失败，防止静默覆盖
...
- 如果是 `CONFLICT_MERGE`，默认走 `original_branch`
- `original_branch`：取消当前阻塞 run，再根据 `featureBranch` / `releaseBranch` 做本地冲突定位与修复
- 如果判断必须直接修改 `releaseBranch`，立即退出并报告用户，由用户决定是否人工介入
...
2. 默认按 `original_branch` 处理
3. 取消当前阻塞 run，避免旧 run 一直挂在云效里
4. `git fetch origin <releaseBranch>`
5. 在当前分支执行 `git merge --no-commit --no-ff origin/<releaseBranch>` 做本地合并演练
6. 根据冲突文件完成修复并提交
7. `git push`
8. 重新执行 `bash /Users/arc/.agents/skills/yunxiao-dev-deploy/scripts/dev_deploy.sh run --wait`
9. 如果判断必须直接修改 `releaseBranch`，则停止并向用户报告：
10. 当前 skill 不把“直接修改 `release/xxx` 并继续 pipeline”视为默认支持能力
11. 需要用户明确决定是否改走人工 release 分支修复
