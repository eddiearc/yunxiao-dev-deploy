...
11. 如果运行详情里出现阻塞动作，先区分是不是 `CONFLICT_MERGE`。如果是分支冲突，默认只支持“在原分支解决”这条路径
12. 在原分支解决时，先取消这个阻塞 run，避免留下悬挂的 `RUNNING/WAITING` 实例；然后拉取 `releaseBranch` 到本地做合并演练，确认具体冲突文件并在当前分支解决，最后重新触发部署
13. 如果判断这次冲突必须直接改 `releaseBranch` 才能继续，则停止自动处理并明确报告给用户；不要把“直接修改 `release/xxx` 并继续 pipeline”当成 skill 默认能力
