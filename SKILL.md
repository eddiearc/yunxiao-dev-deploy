...
14. 处理 `releaseBranch` 冲突时，默认必须新开 **git worktree**；不要直接在当前本地开发分支或当前工作目录里解冲突，避免污染正在开发的分支和未提交现场
15. 默认工作树策略：保留当前开发分支工作区不动，在仓库外或 `.claude/worktrees/` 下创建临时 worktree，检出 `releaseBranch` 后只在该 worktree 内 fetch、merge、resolve、commit、push
16. 如果 worktree 里解决完一轮冲突并 push 到 `releaseBranch`，必须调用 `ExecutePipelineJobAction` 继续当前 run，然后重新拉取 run detail；不能凭一次 `HTTP 200 true` 就判定冲突已结束
17. 如果下一次 run detail 里又出现新的 `CONFLICT_MERGE`，说明云效继续集成后暴露了下一轮冲突；必须继续按同样流程循环处理，直到 run 进入 `SUCCESS` / `FAIL` / `CANCELED` / `ABORTED` / `TERMINATED`
18. `releaseBranch` 路径的目标只是在“当前这一次 run”上解冲突；默认接受它可能是一次性有效，不把“让后续 run 也不再冲突”作为必须目标
19. 不要为了让后续 run 持续通过，就把当前分支合进别人的 `featureBranch` / 原业务分支；这会把不同人的分支历史混在一起，默认禁止
...
Claude Code 的 Bash tool **每次调用不保留 shell 环境变量**。直接 `source ~/.yunxiao/config.sh` 后再另起一个 Bash 调用，`$YUNXIAO_ACCESS_TOKEN` 会丢失。

**正确做法**：在同一个 `bash -c '...'` 内完成 source 和后续操作：

```bash
bash -c '
source ~/.yunxiao/config.sh
source /path/to/project/.yunxiao/project.env 2>/dev/null
export YUNXIAO_ORGANIZATION_ID YUNXIAO_DEV_PIPELINE_ID
bash /path/to/scripts/dev_deploy.sh run
'
```
...
```bash
# 调用1：source 了但变量不会带到调用2
source ~/.yunxiao/config.sh

# 调用2：$YUNXIAO_ACCESS_TOKEN 为空 → InvalidToken
curl -H "x-devops-gp-token: $YUNXIAO_ACCESS_TOKEN" ...
```
...
4. 为这一轮冲突创建临时 git worktree，例如 `.claude/worktrees/yunxiao-<pipelineRunId>`，并在该 worktree 内检出 `releaseBranch`
5. 在 worktree 内执行 `git fetch origin <releaseBranch> <featureBranch>`
6. 在 worktree 内对 `releaseBranch` 执行 `git merge --no-commit --no-ff <featureBranchCommitId>` 做本地合并演练；如果云效 detail 明确给了 `featureBranchCommitId`，优先用 commit id，不要只按分支名 merge
7. 根据冲突文件完成修复并提交到 `releaseBranch`
8. 在 worktree 内 `git push origin HEAD:<releaseBranch>`
9. 调用 `POST /oapi/v1/flow/organizations/{organizationId}/pipelines/{pipelineId}/pipelineRuns/{pipelineRunId}/jobs/{jobId}/action/{actionId}`，相当于页面上的“我已解决完冲突”
10. 再次拉取同一个 `pipelineRunId` 的 run detail
11. 如果 `blocking=[]` 但 run 仍是 `RUNNING`，继续轮询直到终态
12. 如果出现新的 `CONFLICT_MERGE`，回到第 3 步，按最新 `featureBranch` 再处理一轮
13. 如果 run 进入 `SUCCESS` / `FAIL` / `CANCELED` / `ABORTED` / `TERMINATED`，本次冲突处理闭环结束
14. 冲突处理完成后，删除临时 worktree，确保主工作区仍停留在原开发分支
15. 如果判断必须直接修改 `featureBranch` / 原业务分支，则停止并向用户报告
16. 当前 skill 不把“直接修改业务分支并继续 pipeline”视为默认支持能力，更不把“把当前分支合进别人的分支”视为默认支持能力
17. 需要用户明确决定是否改走业务分支修复
